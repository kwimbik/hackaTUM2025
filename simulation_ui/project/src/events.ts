...
// Queue for events targeting branches that don't exist yet
...
export function applyQueuedEvents(branchId: number, timelineOffset: number) {
  ...
  for (const queued of queuedForThisBranch) {
    ...
    const result = generateEventFromAPI(
      queued.eventName,
      queued.year,
      queued.month,
      queued.branchId,  // This is the correct branchId from the queued event
      timelineOffset,
      queued.apiData
    );
    if (result) {
...
export function generateEventFromAPI(
  eventName: string,
  year: number,
  month: number,
  branchId: number,
